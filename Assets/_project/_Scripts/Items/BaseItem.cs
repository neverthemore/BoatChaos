...
public enum WhoCanEnteract          //—писок названий    
{
...
public class BaseItem : MonoBehaviour, IInteractable, IPromtable
{
    ...
    {        
        ...
        if (_whoCanEnteract != WhoCanEnteract.All)  //≈сли не любой персонаж, то далее провер€ем тот ли этот пресонаж
            if (interactor.GetComponent<BaseCharacter>().CharacterName != _whoCanEnteract.ToString()) return; 

        _interactor = interactor;
        IsInteractionAllowed = true;
        //Debug.Log("Ёто " + interactor.GetComponent<BaseCharacter>().CharacterName);\       
    }   
